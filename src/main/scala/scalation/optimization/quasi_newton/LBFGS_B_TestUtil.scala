...
object LBFGS_B_TestUtil:
  ...
  def runLBFGSB (
    ...
    val opt =
      if bounds == null then new LBFGS_B (f, exactLS = exactLS)
  ...
  def compareLS (
    ...
    val resMT =
      if bounds == null then new LBFGS_B (f, exactLS = false).solve (x0)
      else new LBFGS_B (f, exactLS = false, l_u = bounds).solve (x0)

    val res1D =
      if bounds == null then new LBFGS_B (f, exactLS = true).solve (x0)
